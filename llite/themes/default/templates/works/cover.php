<?php 
	...
	$img_file = $no_img_file =  $this->HOST."themes/default/images/no_cover.png";
	...
	if (!empty($this->record)) {
		...
		if (!$hasLocalCover && !empty($this->record->get($field))) {
			...
			$this->addJS("page.post('$ajaxFieldId', 'results/look.for.cover/{$this->record->getId()}', $jsonISBN )");
			}		
		}
	...
	if ($img_file != $no_img_file)
		$this->addJS("
		var pic = $('#$imageFieldId').attr('src');
		$('#work_covers').append('<img src=\"'+pic+'\">');
		");
