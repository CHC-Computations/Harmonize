	...
	foreach ($facets as $k=>$v) {
		if ($v>0) {
			...
			if ($this->buffer->isActiveFacet($facet->solr_index,$k)) {
				...
				$lines[]=
					'<a href="'.$this->buildUri('persons/results/1/'.$this->getUserParam('sort').'/'.$key, $this->GET).'" class="facet js-facet-item active" data-title="'.$k.'" data-count="'.$v.'" >
				...
				if (!empty($facet->child)) {
					...
					$CFD = '<div class="facetCascade" id="facetLink'.$key.'">'.$this->render('persons/facet-cascade-empty.php').'</div>';
					} else
					$CFD = '';
				
				$lines[] = '
					<div class="facetTop" OnMouseOver="facets.place(\''.$key.'\')" OnMouseOut="facets.out(\''.$key.'\')">
					  <a href="'.$this->buildUri('persons/results/1/'.$this->getUserParam('sort').'/'.$key, $this->GET).'" id="facetBase'.$key.'" class="facet js-facet-item" data-title="'.$this->transEsc($k).'" data-count="'.$v.'" >
						<span class="text">'.$tk.'</span>
