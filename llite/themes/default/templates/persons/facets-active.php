...
foreach ($activeFacets as $groupCode=>$arr) {
	...
	$active .= '<div style="padding:5px;">'.$this->transEsc( $this->persons->facets->facetsMenu->$groupCode->name ).':</div>';
	$lp=0;
	foreach ($arr as $k=>$v) {
		$lp++;
		...
		if (!empty($this->persons->facets->facetsMenu->$groupCode->formatter)) $tvalue = $this->helper->{$this->persons->facets->facetsMenu->$groupCode->formatter}($value);
		if ($this->persons->facets->facetsMenu->$groupCode->translated) $tvalue = $this->transEsc($value);
		$key = $this->buffer->createFacetsCode(
							$this->sql, 
							$this->buffer->removeFacet($groupCode, $value)
							);
		
		$active .= '<a href="'.$this->buildUri('persons/results/1/'.$this->getUserParam('sort').'/'.$key).'" class="facet">
