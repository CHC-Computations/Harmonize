...
$livingPlaces = $wiki->getPropIds('P159');
...
$query['q'] = ['field' => 'q', 'value' => 'corporate_wiki:'.$wikiIntId]; // persons_wiki_str_mv - if any role 
...
if (!empty($results))
	foreach ($results as $index=>$places)
		foreach ($places as $place=>$count) {
			$tmp = explode('|',$place);
			if (($tmp[0]<>'')&($tmp[0]<>'not found')) {
				$Tcoor[$tmp[0]] = $tmp[0];
				@$checkedResults[$index][$tmp[0]] += $count;
				$checkedSolrStr[$index][$tmp[0]] = $place;
				} else 
	...
	if (!empty($Tin)) {
		...
		if (!empty($checkedResults))
			foreach ($checkedResults as $index=>$places) {
				...
				if ($this->routeParam[$mch] == 'true') {
					foreach ($places as $placeWikiQ=>$count) {
						if (!empty($Tcoor[$placeWikiQ]['lat'])) {
							...
							$placesOnMap[$gpoint['coor']]['wikiq'] = $gpoint['wikiq'];
							$placesOnMap[$gpoint['coor']]['wikiName'] = $this->helper->formatWiki($gpoint['wikiq']);
							...
							$placesOnMap[$gpoint['coor']]['roles'][$index]['link'] = $this->buildUri('search/results/1/r/'.$this->buffer->createFacetsCode($this->sql, ["$index:\"{$checkedSolrStr[$index][$placeWikiQ]}\"", "corporate_wiki:\"{$wikiIntId}\""]));
				...
				$rightMenu.= '<label class="switch"><input type="checkbox" '.$checked.' id="map_checkbox_'.$mch.'" OnChange="results.maps.addInstitutionRelatations(\''.$wikiId.'\')"><span class="slider slider-'.$color.' round"></span></label> ';
		...
		if ($this->routeParam[$mch] == 'true') {
			...
			if (!empty($livingPlaces)){
				foreach ($livingPlaces as $placeId) {
					...
					if (!empty($Tcoor[$placeWikiQ]['lat'])) {
						...
						$placesOnMap[$gpoint['coor']]['roles'][$index]['title'] = $this->transEsc('Headquarters location');
			...
			$checked = '';	
		$rightMenu .= '<label class="switch"><input type="checkbox" '.$checked.' id="map_checkbox_'.$mch.'" OnChange="results.maps.addInstitutionRelatations(\''.$wikiId.'\')"><span class="slider slider-green round"></span></label> '.$this->transEsc('Residence places').' <span class="badge">'.$this->helper->badgeFormat($lp).'</span>';
		...
		if (!empty($placesOnMap)) 
			foreach ($placesOnMap as $coor=>$place) {
				$pjs = [];
				#echo $this->helper->pre($place);
				$point['link'] = "<h3><a href='{$place['headLink']}'>{$place['wikiName']}</a></h3>";
